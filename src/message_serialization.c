...
void SendMessage_AxisReport(roboszpon_node_t* node) {
    ...
    uint32_t position = FloatAsUint32(node->motor->state.position);
    uint32_t velocity = FloatAsUint32(node->motor->state.velocity);
...
void SendMessage_MotorReport(roboszpon_node_t* node) {
    ...
    uint32_t current = FloatAsUint32(node->motor->state.current);
    uint32_t duty = FloatAsUint32(node->motor->state.duty);
...
void SendMessage_ParameterResponse(uint8_t nodeId, uint8_t paramId,
    ...
    uint32_t valueBits = FloatAsUint32(value);
